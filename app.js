...
function renderParentCerts(){
  ...
  if(!el)return;
  const sorted=[...STATE.profiles].sort((a,b)=>(b.points||0)-(a.points||0));
  if(!sorted.length){el.innerHTML='<p class="text-muted text-sm text-center card">لا يوجد أطفال بعد</p>';return;}
  el.innerHTML=`
    <div class="card mb-8">
      <div class="bold mb-8" style="font-size:1rem">📜 شهادات الأطفال</div>
      <p class="text-xs text-muted mb-10">اختر طفلاً لعرض شهادته وحفظها أو طباعتها كـ PDF</p>
  ...
  listEl.innerHTML=sorted.map((p,i)=>`
    <div style="display:flex;align-items:center;gap:10px;padding:10px;background:var(--surf2);border-radius:12px;margin-bottom:8px">
      <span style="font-size:1.4rem">${medals[i]||'#'+(i+1)}</span>
      <span style="font-size:2rem">${p.avatar}</span>
      <div style="flex:1;min-width:0">
        <div class="bold" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${p.name}</div>
        <div class="text-xs text-muted">${p.points||0} نقطة • ${getLevel(p.points||0).name}</div>
      </div>
...
function showParentCertificate(profile){
  ...
  if(!preview)return;
  const sorted=[...STATE.profiles].sort((a,b)=>(b.points||0)-(a.points||0));
  const ranks=sorted.map((pr,i)=>`<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(240,180,41,0.2);font-size:0.82rem"><span>${['🥇','🥈','🥉'][i]||i+1+'.'} ${pr.avatar} ${escapeHtml(pr.name)}</span><span>${pr.points||0} نقطة</span></div>`).join('');
  const dateStr=new Date().toLocaleDateString('ar-SA',{year:'numeric',month:'long',day:'numeric'});
  const lv=getLevel(profile.points||0);

  preview.innerHTML=`
    <div id="printable-cert" style="background:var(--surf);border:2.5px solid var(--accent);border-radius:18px;padding:20px 16px;text-align:center;margin-top:4px">
      <div style="font-family:'Amiri',serif;font-size:1rem;color:var(--accent);font-weight:700;margin-bottom:8px;padding-bottom:8px;border-bottom:1px solid rgba(240,180,41,0.3)">بِسْمِ اللهِ الرَّحْمَنِ الرَّحِيم 🤲</div>
      <div style="font-size:2rem;margin-bottom:4px">🌙</div>
      <div style="font-family:'Amiri',serif;font-size:1.1rem;color:var(--accent);font-weight:700;margin-bottom:6px">شهادة صائم صغير متميز</div>
      <div style="font-size:2.8rem;margin:6px 0">${profile.avatar}</div>
      <div style="font-size:1.7rem;font-weight:900;color:var(--accent);font-family:'Amiri',serif;margin:6px 0">${escapeHtml(profile.name)}</div>
      <div style="display:inline-block;background:rgba(240,180,41,0.15);border:1px solid var(--accent);color:var(--accent);padding:3px 14px;border-radius:18px;font-size:0.82rem;margin:5px 0">${escapeHtml(lv.name)}</div>
      <div style="margin:8px 0 4px;font-size:0.95rem;font-weight:900;color:var(--accent)">أتم بنجاح صوم شهر رمضان المبارك 1447 هـ</div>
      <div style="font-size:1.8rem;font-weight:900;color:var(--teal);margin:6px 0">${profile.points||0} نقطة</div>
      <div style="font-size:0.75rem;color:var(--muted);margin-bottom:10px">${dateStr}</div>
      <div style="border-top:1px solid rgba(240,180,41,0.3);padding-top:10px;margin-top:6px">
        <div style="font-size:0.8rem;font-weight:700;color:var(--muted);margin-bottom:6px">🏆 ترتيب العائلة</div>
        ${ranks}
      </div>
    </div>
...
function wireButtons(){
  ...
  if(endRam29)endRam29.addEventListener('click',()=>{if(!confirm('إنهاء رمضان بعد 29 يوماً وبدء احتفال العيد؟'))return;STATE.ramadanDays=29;STATE.eidConfirmed=true;save();
