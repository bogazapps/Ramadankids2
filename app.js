...
// ================================================================
// 🎈  BALLOON GAME — يتجدد كل يوم بعد المغرب
// ================================================================
...
const BALLOON_PRIZES=[
  ...
  // جواهر كبيرة (نادرة)
  {type:'gems',icon:'💎',label:'+10 جواهر رائعة! 🌟',val:10,rare:true},
  ...
  {type:'item',icon:'🧭',label:'البوصلة هدية!',val:'compass'},
  ...
  {type:'item',icon:'💎',label:'شارة الماسة! 👑',val:'diamond',rare:true},
  // مهام خاصة
  {type:'task',icon:'🤲',label:'قل سبحان الله 10 مرات',val:'tasbih'},
  {type:'task',icon:'📖',label:'اقرأ صفحة من القرآن الكريم',val:'quran'},
  {type:'task',icon:'😊',label:'ابتسم لأحد أفراد عائلتك',val:'smile'},
  {type:'task',icon:'💧',label:'اشرب كوب ماء الآن',val:'water'},
  {type:'task',icon:'🤝',label:'ساعد أحداً واكسب +25 نقطة!',val:'help',bonus:25},
  {type:'task',icon:'🌙',label:'صلِّ ركعتين نافلة',val:'prayer'},
  {type:'task',icon:'🎁',label:'أهدِ ابتسامة لثلاثة أشخاص',val:'smile3'},
  // فارغ / لا شيء
  {type:'empty',icon:'💨',label:'فارغ! حاول مرة أخرى غداً 😄',val:0},
  {type:'empty',icon:'☁️',label:'حظك أوفر غداً إن شاء الله!',val:0},
  // انفجار (خسارة مضحكة)
...
// أوزان الاحتمالات
const PRIZE_POOLS=[
  {filter:p=>p.type==='points'&&p.val<=30&&!p.rare, w:32},
  {filter:p=>p.type==='points'&&p.val>30&&p.val<=100&&!p.rare, w:14},
  {filter:p=>p.type==='points'&&p.rare, w:3},
  {filter:p=>p.type==='gems'&&p.val<=5, w:18},
  {filter:p=>p.type==='gems'&&p.val>5&&!p.rare, w:7},
  {filter:p=>p.type==='gems'&&p.rare, w:2},
  {filter:p=>p.type==='item'&&!p.rare, w:10},
  {filter:p=>p.type==='item'&&p.rare, w:2},
  {filter:p=>p.type==='task', w:14},
  {filter:p=>p.type==='empty', w:7},
  {filter:p=>p.type==='buzz', w:6},
];
const BUILT_POOLS=PRIZE_POOLS.map(pp=>({pool:BALLOON_PRIZES.filter(pp.filter),w:pp.w}));
...
function weightedPick(rng){
  const total=BUILT_POOLS.reduce((a,b)=>a+b.w,0);
  let r=rng()*total;
  for(const pp of BUILT_POOLS){r-=pp.w;if(r<=0&&pp.pool.length)return pp.pool[Math.floor(rng()*pp.pool.length)];}
  return BALLOON_PRIZES[0];
}
function getDayKey(){
  // Resets after maghrib — if past maghrib use tomorrow's date as key
  ...
  if(td&&td.maghrib){
    ...
    if(now>=mag){const tom=new Date(now);tom.setDate(tom.getDate()+1);return tom.toISOString().split('T')[0];}
  }
  return iso;
}
function genBalloons(dayKey){
  const seed=dayKey.replace(/-/g,'').split('').reduce((a,c,i)=>a+c.charCodeAt(0)*(i+7),0);
  const rng=seededRng(seed);
  const colors=['#ff6b6b','#ffd93d','#6bcb77','#4d96ff','#ff922b','#cc5de8','#f06595','#20c997','#74c0fc','#a9e34b','#ff8787','#845ef7'];
  const emojis=[...BALLOON_EMOJIS].sort(()=>rng()-0.5).slice(0,6);
  return emojis.map((em,i)=>({
    emoji:em,
    color:colors[Math.floor(rng()*colors.length)],
    prize:weightedPick(rng),
    dur:(2.2+rng()*1.8).toFixed(1)+'s',
    del:(rng()*1.4).toFixed(1)+'s',
  }));
}
function applyPrize(p,prize){
  const msgs=[];
  if(prize.type==='points'){
    p.points=(p.points||0)+prize.val;
    const log=dayLog(p);log.pts=(log.pts||0)+prize.val;
    ...
    if(!p.inventory)p.inventory=[];
    if(!p.inventory.includes(prize.val)){
      p.inventory.push(prize.val);
      msgs.push(`${prize.icon} حصلت على ${prize.label}`);
    }else{
      p.gems=(p.gems||0)+8;
      msgs.push(`${prize.icon} المقتنى موجود لديك ← +8 💎 بدلاً عنه!`);
    }
  }else if(prize.type==='task'){
    msgs.push(`${prize.icon} مهمة اليوم: ${prize.label}`);
...
function renderBalloonGame(p){
  ...
  const balloons=genBalloons(dayKey);

  // ── Done state ──
  if(gs.done){
    statusLbl.textContent='أكملت لعبة اليوم! يُجدَّد غداً بعد المغرب 🌙';
    badge.classList.remove('hidden');
    grid.innerHTML=balloons.map((b,i)=>{
      const picked=gs.picks.includes(i);
      return `<button class="balloon-btn ${picked?'balloon-popped':'balloon-faded'}" disabled
        style="--bdur:${b.dur};--bdel:${b.del}">
        <span class="balloon-emoji" style="filter:drop-shadow(0 3px 8px ${b.color})">${b.emoji}</span>
      </button>`;
    }).join('');
    if(gs.prizes.length){
      resultDiv.classList.remove('hidden');
      resultDiv.innerHTML=`<div class="bold text-center mb-8" style="color:var(--accent)">🎁 جوائزك اليوم</div>`+
        gs.prizes.map(msgs=>`<div class="balloon-prize-row">${msgs.map(m=>`<div>${m}</div>`).join('')}</div>`).join('');
    }
    return;
  }
  ...
  const rem=3-gs.picks.length;
  statusLbl.textContent=rem>0?`اختر ${rem===3?'3 بالونات':rem===2?'بالونَين':'بالوناً واحداً'}!`:'رائع! اخترت 3 بالونات 🎉';

  grid.innerHTML=balloons.map((b,i)=>{
    const picked=gs.picks.includes(i);
    return `<button class="balloon-btn ${picked?'balloon-selected':''}" 
      data-bi="${i}" style="--bdur:${b.dur};--bdel:${b.del};--bcol:${b.color}">
      <span class="balloon-emoji" style="filter:drop-shadow(0 4px 10px ${b.color})">${b.emoji}</span>
    </button>`;
  }).join('');

  // Events
  grid.querySelectorAll('.balloon-btn:not(.balloon-selected)').forEach(btn=>{
    btn.addEventListener('click',function(){
      if(gs.done||gs.picks.length>=3)return;
      const idx=parseInt(this.dataset.bi);
      this.classList.add('balloon-pop-anim');
      this.style.pointerEvents='none';
      setTimeout(()=>{
        gs.picks.push(idx);
        const msgs=applyPrize(p,balloons[idx].prize);
        gs.prizes.push(msgs);
        if(gs.picks.length>=3)gs.done=true;
        p.balloonGame[dayKey]=gs;
        save();
        renderBalloonGame(p);
        msgs.forEach(m=>toast&&toast(m));
      },320);
    });
  });
  ...
  if(gs.prizes.length){
    resultDiv.classList.remove('hidden');
    resultDiv.innerHTML=`<div class="bold text-center mb-8" style="color:var(--accent)">🎁 جوائزك حتى الآن</div>`+
      gs.prizes.map(msgs=>`<div class="balloon-prize-row">${msgs.map(m=>`<div>${m}</div>`).join('')}</div>`).join('');
  }else{
