...
function renderCertificate(){
  ...
  // Populate the static elements the HTML already has
  ...
  // Rank badge — individual only, no family table
  const rank   = snap?snap.rank:([...STATE.profiles].sort((a,b)=>(b.points||0)-(a.points||0)).findIndex(x=>x.id===p.id)+1)||1;
  const total  = snap?(snap.familyRanks?snap.familyRanks.length:STATE.profiles.length):STATE.profiles.length;
  const rankMedal=['👸','🤴','👸'][rank-1]||'🏅';
  const rankLabel=rank===1?'الملكة':rank===2?'الملك':rank===3?'الأميرة':'المركز '+rank;
  if(ranksEl){
    ranksEl.innerHTML=`<div style="display:inline-block;background:rgba(56,201,176,0.12);border:1.5px solid var(--teal);color:var(--teal);padding:10px 28px;border-radius:24px;font-size:1.15rem;font-weight:900;margin:4px 0">${rankMedal} ${rankLabel}</div><div style="font-size:0.75rem;color:var(--muted);margin-top:6px">من أصل ${total} مشارك</div>`;
  }
  // Mark cert as seen (clears the home banner)
  if(snap&&!p.certSeen){p.certSeen=true;saveLocal();}
}
...
function buildCertHTML(snap,profile){
  ...
  const days   = snap?snap.days   :(STATE.ramadanDays||30);
  // Individual rank only
  const rank   = snap?snap.rank:([...STATE.profiles].sort((a,b)=>(b.points||0)-(a.points||0)).findIndex(p=>p.id===(profile&&profile.id))+1)||1;
  const total  = snap?(snap.familyRanks?snap.familyRanks.length:STATE.profiles.length):STATE.profiles.length;
  const rankMedal=['👸','🤴','👸'][rank-1]||'🏅';
  const rankLabel=rank===1?'الملكة':rank===2?'الملك':rank===3?'الأميرة':'المركز '+rank;

  return`<div style="background:var(--surf);border:2.5px solid var(--accent);border-radius:18px;padding:24px 18px;text-align:center;max-width:420px;margin:0 auto">
    ...
    <div style="font-family:'Amiri',serif;font-size:1.15rem;color:var(--accent);font-weight:700;margin-bottom:8px">شهادة صائم صغير متميز</div>
    <div style="font-size:3.2rem;margin:8px 0">${avatar}</div>
    <div style="font-size:1.8rem;font-weight:900;color:var(--accent);font-family:'Amiri',serif;margin:4px 0">${escapeHtml(name)}</div>
    ...
    <div style="margin:10px 0 4px;font-size:1rem;font-weight:900;color:var(--text)">أتم بنجاح صيام شهر رمضان المبارك 1447 هـ</div>
    <div style="font-size:0.85rem;color:var(--muted);margin-bottom:8px">${days} يوماً</div>
    <div style="font-size:2.4rem;font-weight:900;color:var(--teal);margin:10px 0">${pts} نقطة ⭐</div>
    <div style="display:inline-block;background:rgba(56,201,176,0.12);border:1.5px solid var(--teal);color:var(--teal);padding:8px 22px;border-radius:24px;font-size:1.05rem;font-weight:900;margin:6px 0">${rankMedal} ${rankLabel}</div>
    <div style="font-size:0.72rem;color:var(--muted);margin-top:4px">من أصل ${total} مشارك</div>
    <div style="font-size:0.72rem;color:var(--muted);margin-top:8px">${dateStr}</div>
  </div>`;
}
