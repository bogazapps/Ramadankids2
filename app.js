...
function buyItem(id){
  ...
  if(id==='gem_bonus'){p.gems=(p.gems||0)+15;toastMsg='💎 +15 جوهرة أُضيفت فوراً!';}
  if(isBuff){
  ...
  if(id==='chest'){
    ...
    p.gems=(p.gems||0)+bonus;
    const cm=document.getElementById('chest-msg');
    if(cm)cm.textContent='+'+bonus+' جوهرة مكافأة! 🎉';
    showModal('m-chest');
...
function renderProfiles(){
  ...
  el.innerHTML=sorted.map((p,i)=>{
    ...
    const editBtn=isMe?`<button class="btn btn-ghost btn-sm" onclick="openAvatarEditor()" style="font-size:0.75rem;padding:4px 10px">✏️ شخصيتي</button>`:'';
