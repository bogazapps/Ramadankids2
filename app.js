...
// ============================================================
// DERIVED STATS (streak / achievements) — recomputed from logs
// ============================================================
function _isSuccessLog(l){
  if(!l) return false;
  // Success if reached any target OR completed to maghrib OR migration-completed
  return !!(l.maghribReached || l.targetReached || l.ateReason==='completed');
}

function computeStreakStats(p){
  // Uses timetable dates to treat missing logs as breaks.
  const tt = (STATE.timetable||[]).slice();
  if(!p || !tt.length) return {streak: (p?.streak||0), maxStreak:(p?.maxStreak||0)};

  const td = todayTT();
  const today = td?.dateISO || todayISO();

  // Consider only dates up to today
  const days = tt
    .filter(d=>d && d.dateISO && d.dateISO<=today)
    .map(d=>d.dateISO)
    .sort();
  ...
  // Current streak: consecutive successes ending at the last successful day (could be today or earlier)
  let streak=0;
  for(let i=days.length-1;i>=0;i--){
    const iso = days[i];
    const l = (p.logs||{})[iso];
    if(_isSuccessLog(l)) streak += 1;
    else if(streak>0) break; // stop after first break following successes
    else continue; // keep scanning until we find the latest success
  }

  return {streak, maxStreak:max};
}

function recomputeDerivedStats(p){
  if(!p) return false;
  const prevS = p.streak||0;
  const prevM = p.maxStreak||0;
  const {streak, maxStreak} = computeStreakStats(p);
  p.streak = streak;
  p.maxStreak = Math.max(prevM||0, maxStreak||0);
  return (p.streak!==prevS) || (p.maxStreak!==prevM);
}
...
function renderAllThemeGrids(){
  ...
  ['theme-grid-parent','theme-grid-modal','theme-grid-main'].forEach(id=>{
    ...
    // Do NOT auto-apply on tap — ask for confirmation first
    el.querySelectorAll('[data-t]').forEach(b=>b.addEventListener('click',()=>requestThemeApply(b.dataset.t)));
  });
}

// Theme apply confirmation
let _pendingTheme = null;
function requestThemeApply(t){
  const th = THEMES.find(x=>x.id===t);
  if(!th) return;
  _pendingTheme = th.id;
  const ic = document.getElementById('theme-confirm-icon');
  const tx = document.getElementById('theme-confirm-text');
  if(ic) ic.textContent = th.icon;
  if(tx) tx.textContent = `هل تريد تطبيق مظهر: ${th.label}؟`;
  showModal('m-theme-confirm');
}
...
function runPostLoadMaintenance(){
  ...
  // Recompute streak + achievements for all profiles (important after any backfill/edits)
  let derivedChanged = false;
  for(const p of (STATE.profiles||[])){
    if(recomputeDerivedStats(p)) derivedChanged = true;
    // Unlock any missed achievements (idempotent)
    checkAch(p);
  }

  if(claimed || backfilled || derivedChanged){
    saveLocal();
    save();
  }
...
function checkAch(p){
  // Always recompute streak from logs so achievements update even when editing/backfilling past days
  recomputeDerivedStats(p);

  const allLogs=Object.values(p.logs||{});
  const success=allLogs.filter(l=>_isSuccessLog(l)).length;
  const maxS = Math.max(p.maxStreak||0, p.streak||0);
  const didAllDeeds = allLogs.some(l=>(l && (l.deeds||[]).length>=GOOD_DEEDS.length));
  const didAllQuests = allLogs.some(l=>(l && (l.quests||[]).length>=QUESTS.length));
  const checks=[
    ['first',success>=1],
    ['s3',maxS>=3],
    ['s7',maxS>=7],
    ['s14',maxS>=14],
    ['pts500',(p.points||0)>=3000],
    ['pts1000',(p.points||0)>=8000],
    ['allDeeds',didAllDeeds],
    ['allQuests',didAllQuests]
  ];
...
function renderProgress(){
  const p=prf();if(!p)return;
  // Keep streak/achievements consistent if past days were modified/backfilled
  recomputeDerivedStats(p);
  checkAch(p);
  const pts=p.points||0;const lv=getLevel(pts);const next=getNextLevel(pts);
...
function wireButtons(){
  ...
  // Theme confirmation
  document.getElementById('btn-theme-yes')?.addEventListener('click',()=>{
    if(_pendingTheme){
      applyTheme(_pendingTheme);
      toast('🎨 تم تطبيق المظهر!');
    }
    _pendingTheme=null;
    closeModal('m-theme-confirm');
  });
  document.getElementById('btn-theme-no')?.addEventListener('click',()=>{
    _pendingTheme=null;
    closeModal('m-theme-confirm');
  });
