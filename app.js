...
const SHOP_ITEMS = [
  ...
  {id:'gem_bonus', name:'مكافأة الجواهر', icon:'💎', cat:'نقاط',  desc:'+ 15 جوهرة مجانية فور الشراء',   price:200},
  // ═══ قوى خاصة ═══
  {id:'grace',     name:'درع الحماية',    icon:'🛡️', cat:'قوى',  desc:'يحمي سجل يوم صعب (درع واحد)',     price:250},
  {id:'chest',     name:'صندوق مفاجأة',  icon:'🎁', cat:'قوى',  desc:'جواهر عشوائية مجانية فور الفتح',  price:150},
...
function renderShop(){
  ...
  const INSTANTS=['dates','honey','treasure','gem_bonus'];
  const BUFFS=['x2','x3'];
...
function buyItem(id){
  ...
  // Consumables — can buy multiple (skip/hint/revive/time_ext)
  const CONSUMABLES=['skip','hint','revive','time_ext'];
  // Instant one-shot rewards
  const INSTANTS=['dates','honey','treasure','gem_bonus'];
  // Daily multiplier buffs
  const BUFFS=['x2','x3'];
  // Regular inventory items (badges/titles/accessories) — one per player
  ...
  // ── Consumables: add to counter ──
  if(isConsumable){if(!p.consumables)p.consumables={};p.consumables[id]=(p.consumables[id]||0)+1;}
  // ── Instant rewards: apply now ──
  if(id==='dates'){p.points=(p.points||0)+30;toast('🌴 +30 نقطة مكافأة مباشرة! 🎉');}
  if(id==='honey'){p.points=(p.points||0)+50;toast('🍯 +50 نقطة حلوة مباشرة! 🎉');}
  if(id==='treasure'){p.points=(p.points||0)+100;toast('💰 +100 نقطة كنز! 🎉');}
  if(id==='gem_bonus'){p.gems=(p.gems||0)+15;toast('💎 +15 جوهرة مجانية! 🎉');}
  // ── Buffs: activate for today ──
  if(isBuff){
    const mult=id==='x2'?2:3;
    p.activeBuff={mult,id,date:todayISO(),name:item.name,icon:item.icon};
    toast(item.icon+' '+item.name+' مفعّل اليوم! كل نقاطك ×'+mult+' 🚀');
  }
  // ── Grace ──
  if(id==='grace'){p.graceTokens=(p.graceTokens||0)+1;toast('🛡️ درع حماية أُضيف!');}
  // ── Chest ──
  if(id==='chest'){const bonus=30+Math.floor(Math.random()*70);p.gems=(p.gems||0)+bonus;const cm=document.getElementById('chest-msg');if(cm)cm.textContent='+'+bonus+' جوهرة مكافأة! 🎉';showModal('m-chest');}
  // ── Normal inventory ──
  if(isInventory){if(!p.inventory)p.inventory=[];p.inventory.push(id);}
  addActivityLog(p,'اشترى '+item.name,-item.price,'approved');save();renderShop();
  if(!isInstant&&!isBuff&&id!=='grace'&&id!=='chest')toast('🎉 اشتريت '+item.name+'!');
}
...
function renderQuizQ(){
  ...
  // Powerup buttons
  const skipCount=cons.skip||0;const hintCount=cons.hint||0;const reviveCount=cons.revive||0;
  const canRevive=QZ.cur>0&&reviveCount>0;
  const powerups=`<div class="quiz-powerups">
    ${skipCount>0?`<button type="button" class="qpwr-btn" id="qpwr-skip" title="تخطّي السؤال">⏭️ <span class="qpwr-cnt">${skipCount}</span></button>`:''}
    ${hintCount>0?`<button type="button" class="qpwr-btn" id="qpwr-hint" title="حذف إجابتين خاطئتين">💡 <span class="qpwr-cnt">${hintCount}</span></button>`:''}
    ${canRevive?`<button type="button" class="qpwr-btn" id="qpwr-revive" title="الرجوع للسؤال السابق">❤️ <span class="qpwr-cnt">${reviveCount}</span></button>`:''}
  </div>`;
  ...
  document.getElementById('qpwr-skip')?.addEventListener('click',()=>{
    const p2=prf();if(!p2||!p2.consumables||!p2.consumables.skip)return;
    p2.consumables.skip--;save();
    toast('⏭️ تخطّيت السؤال!');
    QZ.answers=QZ.answers||[];QZ.answers.push({qid:q.id,correct:false,sel:-1,skipped:true});
    QZ.cur++;QZ.cur>=QZ.qs.length?finishQuiz():renderQuizQ();
  });
  document.getElementById('qpwr-hint')?.addEventListener('click',()=>{
    const p2=prf();if(!p2||!p2.consumables||!p2.consumables.hint)return;
    ...
    toHide.forEach(i=>{opts[i].style.opacity='0.2';opts[i].disabled=true;});
  ...
  document.getElementById('qpwr-revive')?.addEventListener('click',()=>{
    const p2=prf();if(!p2||!p2.consumables||!p2.consumables.revive||QZ.cur===0)return;
    p2.consumables.revive--;save();
    QZ.cur--;
    // Remove last answer
    if(QZ.answers&&QZ.answers.length)QZ.answers.pop();
    // Subtract points for that question
    const prevQ=QZ.qs[QZ.cur];if(prevQ)QZ.score=Math.max(0,QZ.score-(prevQ.p||10));
