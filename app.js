...
const BALLOON_PRIZES=[
  ...
  {type:'gems',icon:'💎',label:'+10 جواهر! 🌟',val:10,rare:true},
  {type:'gems',icon:'💎',label:'+20 جوهرة! 👑',val:20,rare:true},
...
const _BPOOLS=[
  {f:p=>p.type==='points'&&p.val<=30&&!p.rare,w:32},
  {f:p=>p.type==='points'&&p.val>30&&p.val<=100&&!p.rare,w:14},
  {f:p=>p.type==='points'&&p.rare,w:3},
  {f:p=>p.type==='gems'&&p.val<=5,w:18},
  {f:p=>p.type==='gems'&&p.val>5&&!p.rare,w:7},
  {f:p=>p.type==='gems'&&p.rare,w:2},
  {f:p=>p.type==='item'&&!p.rare,w:10},
  {f:p=>p.type==='item'&&p.rare,w:2},
  {f:p=>p.type==='task',w:14},
  {f:p=>p.type==='empty',w:7},
  {f:p=>p.type==='buzz',w:6},
...
function _bGen(dayKey){
  ...
  const cols=['#ff6b6b','#ffd93d','#6bcb77','#4d96ff','#ff922b','#cc5de8','#f06595','#20c997','#74c0fc','#a9e34b','#ff8787','#845ef7'];
  const emojis=[...BALLOON_EMOJIS].sort(()=>rng()-0.5).slice(0,6);
  return emojis.map(em=>({em,col:cols[Math.floor(rng()*cols.length)],prize:_bPick(rng),dur:(2.2+rng()*1.8).toFixed(1)+'s',del:(rng()*1.4).toFixed(1)+'s'}));
}
function _bApply(p,prize){
  ...
  if(prize.type==='points'){
    ...
    else{p.gems=(p.gems||0)+8;msgs.push(prize.icon+' المقتنى موجود ← +8 💎 بدلاً عنه!');}
...
function renderBalloonGame(p){
  ...
  if(!grid.dataset.day||grid.dataset.day!==dayKey){
    grid.dataset.day=dayKey;
    grid.innerHTML=balloons.map((b,i)=>
      '<button type="button" class="balloon-btn" data-bi="'+i+'" style="--bdur:'+b.dur+';--bdel:'+b.del+';--bcol:'+b.col+'">'+
        '<span class="balloon-emoji" style="filter:drop-shadow(0 4px 10px '+b.col+')">'+b.em+'</span>'+
      '</button>'
    ).join('');
    grid.querySelectorAll('.balloon-btn').forEach(function(btn){
      btn.addEventListener('click',function(e){
        e.preventDefault();e.stopPropagation();
        if(gs.done||gs.picks.length>=3)return;
        const idx=parseInt(this.dataset.bi);
        if(gs.picks.includes(idx))return;
        this.style.animation='bPop .32s ease forwards';
        ...
        setTimeout(function(){
          ...
          save();
          _bUpdateGrid(grid,gs);
          _bUpdateResult(resultDiv,gs);
          _bUpdateStatus(statusLbl,badge,gs);
...
function _bUpdateGrid(grid,gs){
  grid.querySelectorAll('.balloon-btn').forEach(function(btn){
    const i=parseInt(btn.dataset.bi);
    const picked=gs.picks.includes(i);
    btn.classList.toggle('balloon-popped',picked);
    btn.classList.toggle('balloon-faded',gs.done&&!picked);
    if(gs.done||picked)btn.style.pointerEvents='none';
  });
}
function _bUpdateResult(resultDiv,gs){
  if(!resultDiv)return;
  if(!gs.prizes.length){resultDiv.classList.add('hidden');return;}
  resultDiv.classList.remove('hidden');
  const title=gs.done?'🎁 جوائزك اليوم':'🎁 جوائزك حتى الآن';
  resultDiv.innerHTML='<div class="bold text-center mb-8" style="color:var(--accent)">'+title+'</div>'+
    gs.prizes.map(function(msgs){return'<div class="balloon-prize-row">'+msgs.map(function(m){return'<div>'+m+'</div>';}).join('')+'</div>';}).join('');
}
function _bUpdateStatus(statusLbl,badge,gs){
  if(!statusLbl)return;
  if(gs.done){statusLbl.textContent='أكملت لعبة اليوم! يُجدَّد بعد المغرب 🌙';if(badge)badge.classList.remove('hidden');}
  else{if(badge)badge.classList.add('hidden');const rem=3-gs.picks.length;statusLbl.textContent=rem===3?'اختر 3 بالونات واكسب جوائز!':rem===2?'اختر بالونَين!':rem===1?'اختر بالوناً واحداً!':'رائع! 🎉';}
}
})();
