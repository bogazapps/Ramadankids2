...
const BALLOON_PRIZES = [
  // === نقاط ===
  {type:'points', icon:'⭐', label:'+5 نقاط', val:5},
  ...
  {type:'points', icon:'⭐', label:'+30 نقاط', val:30},
  {type:'points', icon:'⭐', label:'+50 نقاط مميزة!', val:50},
  {type:'points', icon:'🌟', label:'+100 نقطة كبرى! 🎉', val:100},
  {type:'points', icon:'💫', label:'+200 نقطة ذهبية! 🥇', val:200},
  {type:'points', icon:'🌠', label:'+500 نقطة أسطورية! 👑', val:500, rare:true},
  // === جواهر ===
  {type:'gems', icon:'💎', label:'+1 جوهرة', val:1},
  {type:'gems', icon:'💎', label:'+2 جوهرة', val:2},
  {type:'gems', icon:'💎', label:'+3 جواهر', val:3},
  {type:'gems', icon:'💎', label:'+5 جواهر 💫', val:5},
  {type:'gems', icon:'💎', label:'+10 جواهر رائعة! ✨', val:10},
  {type:'gems', icon:'💎', label:'+20 جوهرة ذهبية! 🌟', val:20, rare:true},
  // === مقتنيات المتجر ===
  {type:'item', icon:'🏮', label:'فانوس رمضان مجاني!', val:'lantern'},
  {type:'item', icon:'⭐', label:'شارة النجمة مجاناً!', val:'star_badge'},
  ...
  {type:'item', icon:'💡', label:'قلب المساعدة مجاناً!', val:'hint'},
  {type:'item', icon:'🪄', label:'عصا السحر هدية!', val:'wand', rare:true},
  {type:'item', icon:'👑', label:'تاج الأبطال! 🎉', val:'crown', rare:true},
  // === مهام مميزة ===
  {type:'task', icon:'🤲', label:'قل سبحان الله 10 مرات', val:'task_tasbih'},
  {type:'task', icon:'📖', label:'اقرأ صفحة من القرآن', val:'task_quran'},
  {type:'task', icon:'😊', label:'ابتسم لأحد أفراد عائلتك', val:'task_smile'},
  {type:'task', icon:'💧', label:'اشرب كوب ماء الآن', val:'task_water'},
  {type:'task', icon:'🤝', label:'ساعد أحد ولك +20 نقطة', val:'task_help', bonus:20},
  {type:'task', icon:'🌙', label:'صلِّ ركعتين نافلة', val:'task_prayer'},
  // === لا شيء / فارغ ===
  {type:'empty', icon:'💨', label:'فارغ! حاول مرة أخرى غداً', val:0},
  {type:'empty', icon:'💨', label:'حظ أوفر غداً! 😄', val:0},
  // === خسارة ===
  {type:'buzz', icon:'💥', label:'انفجر بدون جائزة! 🙈', val:0},
  {type:'buzz', icon:'😵', label:'آخ! لا شيء هذه المرة', val:0},
];

// وزن الاحتمالات: الأكثر شيوعاً أولاً
const PRIZE_WEIGHTS = [
  {pool: BALLOON_PRIZES.filter(p=>p.type==='points' && p.val<=30 && !p.rare), w:35},
  {pool: BALLOON_PRIZES.filter(p=>p.type==='points' && p.val>30 && p.val<=100 && !p.rare), w:15},
  {pool: BALLOON_PRIZES.filter(p=>p.type==='points' && p.rare), w:3},
  {pool: BALLOON_PRIZES.filter(p=>p.type==='gems' && p.val<=5), w:20},
  {pool: BALLOON_PRIZES.filter(p=>p.type==='gems' && p.val>5 && !p.rare), w:8},
  {pool: BALLOON_PRIZES.filter(p=>p.type==='gems' && p.rare), w:2},
  {pool: BALLOON_PRIZES.filter(p=>p.type==='item' && !p.rare), w:10},
  {pool: BALLOON_PRIZES.filter(p=>p.type==='item' && p.rare), w:2},
  {pool: BALLOON_PRIZES.filter(p=>p.type==='task'), w:15},
  {pool: BALLOON_PRIZES.filter(p=>p.type==='empty'), w:8},
  {pool: BALLOON_PRIZES.filter(p=>p.type==='buzz'), w:7},
];
...
function getBalloonDayKey(){
  // day key based on today's date, resets after maghrib
  const td=todayTT();
  const now=new Date();
  const todayDate=todayISO();
  if(td&&td.maghrib){
    const mag=parseT(td.maghrib,todayDate);
    if(now<mag) return todayDate; // before maghrib → today's key
  }
  return todayDate;
}

function getMaghribBalloonKey(){
  // after maghrib this becomes tomorrow's key
  const td=todayTT();
  const now=new Date();
  const todayDate=todayISO();
  if(td&&td.maghrib){
...
function weightedPick(rng){
  const total=PRIZE_WEIGHTS.reduce((a,b)=>a+b.w,0);
  let r=rng()*total;
  for(const pw of PRIZE_WEIGHTS){
    r-=pw.w;
    if(r<=0&&pw.pool.length){
      return pw.pool[Math.floor(rng()*pw.pool.length)];
    }
  }
  return BALLOON_PRIZES[0];
}

function generateDailyBalloons(dayKey){
  const seed=dayKey.replace(/-/g,'').split('').reduce((a,c)=>a+c.charCodeAt(0),0);
  const rng=seededRand(seed);
  const balloons=[];
  const colors=['#ff6b6b','#ffd93d','#6bcb77','#4d96ff','#ff922b','#cc5de8','#f06595','#20c997','#74c0fc','#a9e34b'];
  const emojis=BALLOON_EMOJIS.slice().sort(()=>rng()-0.5).slice(0,6);
  for(let i=0;i<6;i++){
    balloons.push({
      emoji:emojis[i],
      color:colors[Math.floor(rng()*colors.length)],
      prize:weightedPick(rng),
      floatDur:(2.5+rng()*1.5).toFixed(1)+'s',
      floatDel:(rng()*1.5).toFixed(1)+'s',
    });
  }
  return balloons;
}

function applyBalloonPrize(p,prize){
  const log=dayLog(p);
  const msgs=[];
  if(prize.type==='points'){
    p.points=(p.points||0)+prize.val;
    log.pts=(log.pts||0)+prize.val;
    msgs.push(`${prize.icon} ربحت ${prize.label}!`);
    checkAch(p);
  } else if(prize.type==='gems'){
    p.gems=(p.gems||0)+prize.val;
    msgs.push(`${prize.icon} ربحت ${prize.label}!`);
  } else if(prize.type==='item'){
    const inv=p.inventory||(p.inventory=[]);
    if(!inv.includes(prize.val)){
      inv.push(prize.val);
      msgs.push(`${prize.icon} حصلت على ${prize.label}!`);
    } else {
      // item already owned → give 10 gems instead
      p.gems=(p.gems||0)+10;
      msgs.push(`${prize.icon} المقتنى موجود لديك ← +10 💎 عوضاً عنه!`);
    }
  } else if(prize.type==='task'){
    msgs.push(`${prize.icon} مهمتك: ${prize.label}`);
    if(prize.bonus){p.points=(p.points||0)+prize.bonus;msgs.push(`(أكملها واكسب +${prize.bonus} ⭐)`)}
  } else if(prize.type==='empty'){
    msgs.push(`${prize.icon} ${prize.label}`);
  } else if(prize.type==='buzz'){
    msgs.push(`${prize.icon} ${prize.label}`);
  }
  save();
  return msgs;
...
function renderBalloonGame(p){
  ...
  const refreshBadge=document.getElementById('balloon-refresh-badge');
  if(!grid||!resultDiv||!p) return;

  const dayKey=getMaghribBalloonKey();
  const balloonState=p.balloonGame||(p.balloonGame={});
  const todayState=balloonState[dayKey]||(balloonState[dayKey]={picks:[],prizes:[],done:false});
  const balloons=generateDailyBalloons(dayKey);

  if(todayState.done){
    // Show completed state
    grid.innerHTML=balloons.map((b,i)=>{
      const picked=todayState.picks.includes(i);
      return `<button class="balloon-btn ${picked?'popped':'not-selected'}" disabled style="--bf-dur:${b.floatDur};--bf-del:${b.floatDel}">
        <span class="balloon-emoji" style="filter:drop-shadow(0 3px 8px ${b.color})">${b.emoji}</span>
      </button>`;
    }).join('');
    statusLbl.textContent='أكملت لعبة اليوم! 🎉';
    refreshBadge.classList.remove('hidden');
    // Show prizes
    if(todayState.prizes.length){
      resultDiv.classList.remove('hidden');
      resultDiv.innerHTML='<div class="bold mb-8 text-center">🎁 جوائزك اليوم</div>'+
        todayState.prizes.map(msgs=>
          `<div class="balloon-prize-item"><span class="balloon-prize-icon">🎈</span><div>${msgs.join('<br>')}</div></div>`
        ).join('');
    }
    return;
  }
  ...
  // Remaining picks
  const remaining=3-todayState.picks.length;
  statusLbl.textContent=remaining>0?`اختر ${remaining} بالون${remaining===1?'اً':remaining===2?'ين':'ات'}!`:'أكملت الاختيار!';

  grid.innerHTML=balloons.map((b,i)=>{
    const picked=todayState.picks.includes(i);
    return `<button class="balloon-btn ${picked?'selected':''}" data-bi="${i}" 
      style="--bf-dur:${b.floatDur};--bf-del:${b.floatDel};${picked?'border-color:'+b.color:''}">
      <span class="balloon-emoji" style="filter:drop-shadow(0 3px 8px ${b.color})">${b.emoji}</span>
    </button>`;
  }).join('');

  // Events
  grid.querySelectorAll('.balloon-btn:not(.selected)').forEach(btn=>{
    btn.addEventListener('click',()=>{
      if(todayState.done) return;
      if(todayState.picks.length>=3) return;
      const idx=parseInt(btn.dataset.bi);
      btn.classList.add('popped');
      setTimeout(()=>{
        todayState.picks.push(idx);
        const prize=balloons[idx].prize;
        const msgs=applyBalloonPrize(p,prize);
        todayState.prizes.push(msgs);
        if(todayState.picks.length>=3) todayState.done=true;
        p.balloonGame=balloonState;
        save();
        renderBalloonGame(p);
        // Show result toast
  ...
  // Show cumulative prizes so far
  if(todayState.prizes.length>0){
    resultDiv.classList.remove('hidden');
    resultDiv.innerHTML='<div class="bold mb-8 text-center">🎁 جوائزك حتى الآن</div>'+
      todayState.prizes.map(msgs=>
        `<div class="balloon-prize-item"><span class="balloon-prize-icon">🎈</span><div>${msgs.join('<br>')}</div></div>`
      ).join('');
  } else {
