...
const SHOP_ITEMS = [
  ...
  // ═══ مكافآت النقاط ═══
  {id:'dates',     name:'طبق التمر',      icon:'🌴', cat:'نقاط',  desc:'+ 30 نقطة مباشرة فور الشراء',    price:30},
  {id:'honey',     name:'جرّة العسل',     icon:'🍯', cat:'نقاط',  desc:'+ 50 نقطة حلوة فور الشراء',      price:60},
  {id:'x2',        name:'نقاط مضاعفة',   icon:'⚡', cat:'نقاط',  desc:'كل نقاط المسابقة ×2 اليوم كله',   price:200},
  {id:'x3',        name:'نقاط ثلاثية',   icon:'🚀', cat:'نقاط',  desc:'كل نقاط المسابقة ×3 اليوم كله',   price:400},
  {id:'treasure',  name:'صندوق الكنز',   icon:'💰', cat:'نقاط',  desc:'+ 100 نقطة مباشرة فور الشراء',    price:120},
  {id:'pts_boost', name:'دفعة النقاط',   icon:'🚀', cat:'نقاط',  desc:'+ 200 نقطة كبرى فور الشراء',       price:250},
  // ═══ قوى خاصة ═══
  {id:'grace',     name:'درع الحماية',    icon:'🛡️', cat:'قوى',  desc:'يحمي سجل يوم صعب (درع واحد)',     price:150},
  {id:'chest',     name:'صندوق مفاجأة',  icon:'🎁', cat:'قوى',  desc:'50-200 نقطة عشوائية فور الفتح',   price:80},
  {id:'double_chance', name:'فرصة ثانية',  icon:'🎯', cat:'قوى',  desc:'إذا أخطأت — اختر مرة ثانية في نفس السؤال', price:60},
  {id:'hint',      name:'قلب المساعدة',   icon:'💡', cat:'قوى',  desc:'يحذف إجابتين خاطئتين في سؤال',    price:40},
  {id:'revive',    name:'حياة إضافية',    icon:'❤️', cat:'قوى',  desc:'ارجع للسؤال السابق وصحّح إجابتك', price:80},
  {id:'skip',      name:'تخطّي سؤال',     icon:'⏭️', cat:'قوى',  desc:'تخطّ سؤالاً صعباً وتحصل على نقاطه', price:50},
  // ═══ اكسسوارات الشخصية ═══
...
async function save(){
  ...
  _saveTimer = setTimeout(async()=>{
    beginBusy();
    try{
      // NEVER upload currentId, theme, or quizQuestions — all per-device
      // quizQuestions are large (63KB) and loaded fresh from questions.json on each device
      const {currentId:_cid, theme:_th, quizQuestions:_qq, ...stateToSync} = STATE;
...
function mergeState(loaded){
  ...
  // Preserve locally-fetched quizQuestions — never let remote state overwrite them
  // (they're large and excluded from Supabase; each device loads from questions.json)
  const prevQQ=STATE.quizQuestions&&STATE.quizQuestions.length>10?STATE.quizQuestions:null;
  // Strip quizQuestions from remote payload before merging
  const {quizQuestions:_remoteQQ,...remoteRest}=loaded;
  STATE=Object.assign({...STATE},remoteRest);
  if(prevQQ)STATE.quizQuestions=prevQQ;
...
function renderHome(){
  ...
  renderBalloonGame(p);
  // "بابا" test account: show return-to-start button
  const babaBtn=document.getElementById('btn-baba-return');
  if(babaBtn){babaBtn.style.display=(p.name==='بابا'||p.name==='baba')?'':'none';}
  // Active buff banner
  ...
  // Eid certificate banner — shows to kids when parent has confirmed end of Ramadan
  let eidBanner=document.getElementById('eid-cert-banner');
  if(STATE.eidConfirmed&&!p.certSeen){
    if(!eidBanner){
      eidBanner=document.createElement('div');
      eidBanner.id='eid-cert-banner';
      eidBanner.style.cssText='background:linear-gradient(135deg,rgba(240,180,41,0.22),rgba(56,201,176,0.15));border:2px solid var(--accent);border-radius:16px;padding:14px 16px;margin:10px 0;display:flex;align-items:center;gap:12px;cursor:pointer';
      eidBanner.innerHTML=`<div style="font-size:2.2rem">🏅</div><div style="flex:1"><div class="bold" style="color:var(--accent);font-size:1rem">شهادتك جاهزة! 🎉</div><div class="text-xs text-muted">انتهى رمضان — اضغط لعرض شهادة إنجازك</div></div><span style="color:var(--accent);font-size:1.3rem">←</span>`;
      eidBanner.addEventListener('click',()=>showScreen('certificate'));
      // Insert after the timer card / before balloon game card
      const balloonCard=document.getElementById('balloon-game-card');
      if(balloonCard&&balloonCard.parentNode)balloonCard.parentNode.insertBefore(eidBanner,balloonCard);
      else document.getElementById('screen-home').appendChild(eidBanner);
    }
    eidBanner.style.display='flex';
  } else if(eidBanner){
    eidBanner.style.display='none';
  }
}
...
function renderShop(){
  ...
  // Group by category
  const CONSUMABLES=['skip','hint','revive','double_chance'];
...
function buyItem(id){
  ...
  if(id==='double_chance'){
    if(!p.consumables)p.consumables={};
    p.consumables.double_chance=(p.consumables.double_chance||0)+1;
    toastMsg='🎯 فرصة ثانية أُضيفت! لديك '+(p.consumables.double_chance)+' قطعة';
  }
  if(id==='chest'){
...
function renderCertificate(){
  const p=prf();if(!p)return;
  // Use stored snapshot if Eid is confirmed — preserves final points/rank
  const snap=STATE.certSnapshots&&STATE.certSnapshots[p.id]||null;

  document.getElementById('cert-av').textContent   = snap?snap.avatar:p.avatar;
  document.getElementById('cert-name').textContent = snap?snap.name:p.name;
  document.getElementById('cert-level').textContent= snap?snap.level:getLevel(p.points||0).name;
  document.getElementById('cert-pts').textContent  = snap?snap.points:(p.points||0);
  document.getElementById('cert-date').textContent = snap?snap.date:new Date().toLocaleDateString('ar-SA',{year:'numeric',month:'long',day:'numeric'});

  const days=snap?snap.days:(STATE.ramadanDays||30);
  const msgEl=document.getElementById('cert-msg');
  if(msgEl)msgEl.textContent=`أتم بنجاح صيام شهر رمضان المبارك 1447 هـ (${days} يوماً)`;

  const ranksEl=document.getElementById('cert-ranks');
  if(ranksEl){
    if(snap&&snap.familyRanks){
      ranksEl.innerHTML=snap.familyRanks.map(pr=>`<div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid var(--bdr);font-size:0.88rem"><span>${pr.medal} ${pr.avatar} ${escapeHtml(pr.name)}</span><span>${pr.points} نقطة</span></div>`).join('');
    }else{
      const sorted=[...STATE.profiles].sort((a,b)=>(b.points||0)-(a.points||0));
      ranksEl.innerHTML=sorted.map((pr,i)=>`<div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid var(--bdr);font-size:0.88rem"><span>${['🥇','🥈','🥉'][i]||i+1+'.'} ${pr.avatar} ${escapeHtml(pr.name)}</span><span>${pr.points||0} نقطة</span></div>`).join('');
    }
  }

  // Mark cert as seen (clears the home banner)
  if(snap){p.certSeen=true;saveLocal();}
}
...
function openParentTab(tab){
  ['children','deeds','fasting','questions','rewards','settings','certs'].forEach(t=>{
  ...
  if(tab==='certs')renderParentCerts();
}

// ============================================================
// CERTIFICATE SNAPSHOTS — generated when parent ends the game
// Stored in STATE.certSnapshots[profileId] so every device sees them
// ============================================================
function generateCertSnapshots(){
  if(!STATE.profiles||!STATE.profiles.length)return;
  if(!STATE.certSnapshots)STATE.certSnapshots={};
  const sorted=[...STATE.profiles].sort((a,b)=>(b.points||0)-(a.points||0));
  const dateStr=new Date().toLocaleDateString('ar-SA',{year:'numeric',month:'long',day:'numeric'});
  sorted.forEach((p,i)=>{
    STATE.certSnapshots[p.id]={
      name:p.name,avatar:p.avatar,
      points:p.points||0,
      level:getLevel(p.points||0).name,
      rank:i+1,
      medal:['🥇','🥈','🥉'][i]||(i+1)+'.)',
      date:dateStr,
      days:STATE.ramadanDays||30,
      familyRanks:sorted.map((pr,ri)=>({avatar:pr.avatar,name:pr.name,points:pr.points||0,medal:['🥇','🥈','🥉'][ri]||(ri+1)+'.'}) )
    };
    // Queue celebration in kid's inbox so they see it on next open
    if(!p.inboxCelebrations)p.inboxCelebrations=[];
    // Only add once
    if(!p.inboxCelebrations.find(c=>c.type==='eid_cert')){
      p.inboxCelebrations.unshift({type:'eid_cert',time:Date.now(),text:'🎉 انتهى رمضان! شهادتك جاهزة — تفضل بفتحها!'});
    }
  });
}

// ============================================================
// PARENT CERTIFICATES TAB
// ============================================================
function renderParentCerts(){
  const el=document.getElementById('ptab-certs-content');
  if(!el)return;
  const sorted=[...STATE.profiles].sort((a,b)=>(b.points||0)-(a.points||0));
  if(!sorted.length){
    el.innerHTML='<div class="card"><p class="text-muted text-sm text-center">لا يوجد أطفال بعد</p></div>';
    return;
  }

  const eidDone=STATE.eidConfirmed;
  const medals=['🥇','🥈','🥉'];

  el.innerHTML=`
    ${!eidDone?`<div class="card mb-8" style="border:1.5px solid var(--accent);background:rgba(240,180,41,0.07)">
      <div class="bold mb-6" style="color:var(--accent)">⚠️ الشهادات تُولَّد عند إنهاء رمضان</div>
      <p class="text-xs text-muted mb-8">اذهب لـ ⚙️ الإعدادات واضغط زر إنهاء رمضان، ثم عد هنا لعرض الشهادات.</p>
    </div>`:''}
    <div class="card mb-8">
      <div class="bold mb-4" style="font-size:1rem">📜 شهادات الأطفال</div>
      <p class="text-xs text-muted mb-10">اضغط "عرض" لمعاينة الشهادة وطباعتها كـ PDF</p>
      <div id="parent-certs-list"></div>
    </div>
    <div id="parent-cert-preview"></div>`;

  const listEl=document.getElementById('parent-certs-list');
  listEl.innerHTML=sorted.map((p,i)=>{
    const snap=STATE.certSnapshots&&STATE.certSnapshots[p.id];
    return`<div style="display:flex;align-items:center;gap:10px;padding:10px;background:var(--surf2);border-radius:12px;margin-bottom:8px">
      <span style="font-size:1.4rem">${medals[i]||'#'+(i+1)}</span>
      <span style="font-size:2rem">${p.avatar}</span>
      <div style="flex:1;min-width:0">
        <div class="bold" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escapeHtml(p.name)}</div>
        <div class="text-xs text-muted">${snap?snap.points:p.points||0} نقطة • ${snap?escapeHtml(snap.level):escapeHtml(getLevel(p.points||0).name)}</div>
      </div>
      <button class="btn btn-primary btn-sm" data-certpid="${p.id}">📜 عرض</button>
    </div>`;
  }).join('');

  listEl.querySelectorAll('[data-certpid]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const pid=btn.dataset.certpid;
      const profile=STATE.profiles.find(x=>x.id===pid);
      if(profile)showParentCertificate(profile);
    });
  });
}

function buildCertHTML(snap,profile){
  // snap may be null (no eid confirmed yet) — fall back to live data
  const sorted=[...STATE.profiles].sort((a,b)=>(b.points||0)-(a.points||0));
  const name   = snap?snap.name   :(profile?profile.name:'—');
  const avatar = snap?snap.avatar :(profile?profile.avatar:'🌙');
  const pts    = snap?snap.points :(profile?profile.points||0:0);
  const lv     = snap?snap.level  :(profile?getLevel(profile.points||0).name:'—');
  const dateStr= snap?snap.date   :new Date().toLocaleDateString('ar-SA',{year:'numeric',month:'long',day:'numeric'});
  const days   = snap?snap.days   :(STATE.ramadanDays||30);
  const ranks  = snap
    ? snap.familyRanks.map((pr,i)=>`<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(240,180,41,0.2);font-size:0.82rem"><span>${pr.medal} ${pr.avatar} ${escapeHtml(pr.name)}</span><span>${pr.points} نقطة</span></div>`).join('')
    : sorted.map((pr,i)=>`<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(240,180,41,0.2);font-size:0.82rem"><span>${['🥇','🥈','🥉'][i]||(i+1)+'.'} ${pr.avatar} ${escapeHtml(pr.name)}</span><span>${pr.points||0} نقطة</span></div>`).join('');

  return`<div style="background:var(--surf);border:2.5px solid var(--accent);border-radius:18px;padding:20px 16px;text-align:center">
    <div style="font-family:'Amiri',serif;font-size:1rem;color:var(--accent);font-weight:700;margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid rgba(240,180,41,0.3)">بِسْمِ اللهِ الرَّحْمَنِ الرَّحِيم 🤲</div>
    <div style="font-size:2.2rem;margin-bottom:4px">🌙</div>
    <div style="font-family:'Amiri',serif;font-size:1.15rem;color:var(--accent);font-weight:700;margin-bottom:6px">شهادة صائم صغير متميز</div>
    <div style="font-size:3rem;margin:8px 0">${avatar}</div>
    <div style="font-size:1.7rem;font-weight:900;color:var(--accent);font-family:'Amiri',serif;margin:4px 0">${escapeHtml(name)}</div>
    <div style="display:inline-block;background:rgba(240,180,41,0.15);border:1px solid var(--accent);color:var(--accent);padding:4px 16px;border-radius:20px;font-size:0.85rem;margin:6px 0">${escapeHtml(lv)}</div>
    <div style="margin:10px 0 6px;font-size:1rem;font-weight:900;color:var(--text)">أتم بنجاح صيام شهر رمضان المبارك 1447 هـ</div>
    <div style="font-size:0.88rem;color:var(--muted);margin-bottom:6px">(${days} يوماً)</div>
    <div style="font-size:2rem;font-weight:900;color:var(--teal);margin:8px 0">${pts} نقطة ⭐</div>
    <div style="font-size:0.75rem;color:var(--muted);margin-bottom:12px">${dateStr}</div>
    <div style="border-top:1px solid rgba(240,180,41,0.3);padding-top:10px">
      <div style="font-size:0.85rem;font-weight:700;color:var(--muted);margin-bottom:8px">🏆 ترتيب العائلة</div>
      ${ranks}
    </div>
  </div>`;
}

function showParentCertificate(profile){
  const preview=document.getElementById('parent-cert-preview');
  if(!preview)return;
  const snap=STATE.certSnapshots&&STATE.certSnapshots[profile.id]||null;
  preview.innerHTML=`
    <div id="printable-cert" style="margin-top:6px">${buildCertHTML(snap,profile)}</div>
    <div style="display:flex;gap:8px;margin-top:10px">
      <button class="btn btn-primary btn-sm" style="flex:1" id="btn-cert-print">🖨️ طباعة / حفظ PDF</button>
      <button class="btn btn-ghost btn-sm" style="flex:1" id="btn-cert-close-preview">✕ إغلاق</button>
    </div>`;
  document.getElementById('btn-cert-print')?.addEventListener('click',()=>window.print());
  document.getElementById('btn-cert-close-preview')?.addEventListener('click',()=>{preview.innerHTML='';});
...
function renderFastingTab(){
  ...
  document.getElementById('btn-update-fasting').onclick=()=>{
    ...
    // FIX: use dayLog() to ensure proper initialization of the day log
    // Previously used p.logs[today]={} which created a broken empty object
    const log=dayLog(p,today);
    if(selectedFastingReason==='completed'){
      // Parent confirms the kid completed their fast — remove any broke-reason and mark success
      delete log.ateReason;
      if(!log.maghribReached){
        log.targetReached=true;
        log.maghribReached=true;
        const pts=150;
        log.pts=(log.pts||0)+pts;
        awardPoints(p,pts,'إتمام الصيام (تأكيد الوالد)',{competition:true,gems:true,silent:true});
        addActivityLog(p,'إتمام الصيام (تأكيد الوالد)',pts,'approved');
      }
    } else {
      // Parent is recording that the kid broke their fast
      log.ateReason=selectedFastingReason;
    }
...
function renderQuizQ(){
  ...
  const dcCount=cons.double_chance||0;
  const canRevive=QZ.cur>0&&reviveCount>0;
  // Only show powerups if kid owns them
  const powerups=(skipCount>0||hintCount>0||canRevive||dcCount>0)?`<div class="quiz-powerups">
    ${skipCount>0?`<button type="button" class="qpwr-btn" id="qpwr-skip" title="تخطّ السؤال وتحصل على نقاطه">⏭️ <span class="qpwr-cnt">${skipCount}</span></button>`:''}
    ${hintCount>0?`<button type="button" class="qpwr-btn" id="qpwr-hint" title="احذف إجابتين خاطئتين">💡 <span class="qpwr-cnt">${hintCount}</span></button>`:''}
    ${dcCount>0?`<button type="button" class="qpwr-btn" id="qpwr-dc" title="فرصة ثانية إذا أخطأت">🎯 <span class="qpwr-cnt">${dcCount}</span></button>`:''}
    ${canRevive?`<button type="button" class="qpwr-btn" id="qpwr-revive" title="ارجع للسؤال السابق">❤️ <span class="qpwr-cnt">${reviveCount}</span></button>`:''}
  </div>`:'';
  ...
  // ── Double Chance: auto-activates on next wrong answer ──
  document.getElementById('qpwr-dc')?.addEventListener('click',()=>{
    toast('🎯 الفرصة الثانية مفعّلة! ستُستخدم تلقائياً إذا أخطأت.');
    document.getElementById('qpwr-dc')?.remove();
  });
  // ── Revive: go back ──
...
function answerQ(sel){
  ...
  // If wrong and has double_chance — offer second try
  const dc=(p&&p.consumables&&p.consumables.double_chance)||0;
  if(!correct && dc>0 && !QZ._usedDoubleChance){
    QZ._usedDoubleChance=true;
    p.consumables.double_chance--;save();
    const fb=document.getElementById('quiz-fb');fb.className='quiz-fb wrong';
    fb.innerHTML=`❌ خطأ! لديك فرصة ثانية 🎯 <br><small style="color:var(--teal)">استُهلكت فرصة ثانية (${p.consumables.double_chance} باقية)</small>`;
    fb.classList.remove('hidden');
    document.querySelectorAll('.quiz-opt').forEach((b,i)=>{if(i===sel)b.classList.add('wrong');});
    setTimeout(()=>{
      fb.classList.add('hidden');
      // Re-enable remaining options except the wrong one
      document.querySelectorAll('.quiz-opt').forEach((b,i)=>{if(i!==sel){b.disabled=false;b.classList.remove('opt-pending');}});
      // Re-show confirmation on re-tap
    },1500);
    return;
  }
  QZ._usedDoubleChance=false;
...
// ============================================================
// IMPORTANT: keep APP_VERSION in sync with service-worker.js SW_VERSION.
// This prevents some devices (especially iOS / kids tablets) from getting stuck on an old SW due to HTTP caching.
...
if('serviceWorker' in navigator){
  window.addEventListener('load', async ()=>{
    try{
      // updateViaCache:'none' forces the browser to re-fetch the SW script instead of serving it from HTTP cache.
      // The version query string helps on devices/CDNs that aggressively cache service-worker.js.
      const reg = await navigator.serviceWorker.register(`./service-worker.js?${APP_VERSION}`, { updateViaCache:'none' });

      // Ask for an update right away (helps when the app stays open for long periods)
      try{ await reg.update(); }catch(e){}

      // Periodic update checks + when app/tab becomes visible
      const updateSW = async ()=>{ try{ await reg.update(); }catch(e){} };
      setInterval(updateSW, 30*60*1000);
      document.addEventListener('visibilitychange', ()=>{ if(!document.hidden) updateSW(); });
      ...
      reg.addEventListener('updatefound',()=>{
        ...
        nw.addEventListener('statechange',()=>{
          if(nw.state==='installed' && navigator.serviceWorker.controller){
            // New version is ready — activate it now
            nw.postMessage('SKIP_WAITING');
          }
        });
      });

      // When the new SW takes control, reload once to get new index/app/styles
...
function wireButtons(){
  ...
  document.getElementById('btn-s0').addEventListener('click',()=>{
    ...
    if(!name){toast('✏️ اكتب اسمك أولاً');return}
    if(!age||age<3||age>16){toast('🔢 العمر يجب أن يكون بين 3 و 16');return}
    // Uniqueness check — case-insensitive
    const nameLower=name.trim().toLowerCase();
    const duplicate=(STATE.profiles||[]).some(p=>p.name.trim().toLowerCase()===nameLower);
    if(duplicate){toast('⚠️ يوجد ملف بهذا الاسم بالفعل! اختر اسماً مختلفاً');return}
  ...
  document.getElementById('btn-baba-return')?.addEventListener('click',()=>showScreen('welcome'));
  ...
  if(endRam29)endRam29.addEventListener('click',()=>{if(!confirm('إنهاء رمضان بعد 29 يوماً وبدء احتفال العيد؟'))return;STATE.ramadanDays=29;STATE.eidConfirmed=true;
    // Generate certificate snapshots for all kids at this exact moment
    generateCertSnapshots();
    save();
    // FIX: populate winner info (was missing, causing zeros + no name)
    const sorted=[...STATE.profiles].sort((a,b)=>(b.points||0)-(a.points||0));
    const winner=sorted[0];
    if(winner){document.getElementById('win-av').textContent=winner.avatar;document.getElementById('win-name').textContent=winner.name;document.getElementById('win-pts').textContent=(winner.points||0)+' نقطة';}
    showModal('m-eid-cel');confetti();setTimeout(confetti,800);setTimeout(confetti,1600);toast('🎉 عيد مبارك!');});
  if(cont30)cont30.addEventListener('click',()=>{if(!confirm('إتمام 30 يوم — توليد الشهادات وإنهاء رمضان؟'))return;
    STATE.ramadanDays=30;STATE.eidConfirmed=true;
    generateCertSnapshots();
    save();
    const sorted=[...STATE.profiles].sort((a,b)=>(b.points||0)-(a.points||0));
    const winner=sorted[0];
    if(winner){document.getElementById('win-av').textContent=winner.avatar;document.getElementById('win-name').textContent=winner.name;document.getElementById('win-pts').textContent=(winner.points||0)+' نقطة';}
    showModal('m-eid-cel');confetti();setTimeout(confetti,800);setTimeout(confetti,1600);toast('🌙 أتممتم الثلاثين! عيد مبارك! 🎉');});
